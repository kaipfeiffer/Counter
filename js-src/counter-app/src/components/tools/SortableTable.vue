  ...
  <table v-if="headers">
    ...
    <tbody v-if="rows">
      <tr v-for="row in getPage">
        <td v-for="td in headers" :class="row[td.column]?.class">
          <span v-if="td.column" :class="td.class">
            {{
              "object" === typeof row[td.column]
                ? row[td.column].value
                : row[td.column]
            }}
          </span>
          <span v-if="td.links">
            <RouterLink
              v-for="link in td.links"
              :to="{ name: link.route, params: { id: row[link.index] } }"
...
export default defineComponent({
  ...
  setup: (props) => {
    ...
    const getColumnCount = computed(() => {
      let rows = props.head ? props.head : props.rows;
      let count = rows?.length ? rows.length : 1;
      return count;
    });
    const headers = computed(() => {
      let headers = props.head ? props.head : createHeaders();
      return headers;
    });
    const getPage = computed(() => {
      let rows = props.rows.slice(
        (currentPage.value - 1) * settings.pageSize,
        currentPage.value * settings.pageSize
      );
    ...
    const start = computed(() => {
      return props.rows?.length
        ? (currentPage.value - 1) * settings.pageSize + 1
        : 0;
    });
    ...
    const end = computed(() => {
      ...
      return props.rows?.length && props.rows?.length > last
        ? last
        : props.rows?.length;
    });
    ...
    const getTitle = computed(() => {
      ...
      last =
        props.rows?.length && props.rows?.length > last
          ? last
          : props.rows?.length;
    ...
    const setPage = (index) => {
      switch (index) {
        ...
        case 1: {
          currentPage.value =
            currentPage.value <= props.rows?.length / settings.pageSize
              ? currentPage.value + 1
              : Math.floor(props.rows?.length / settings.pageSize) + 1;
          break;
        }
        default: {
          currentPage.value =
